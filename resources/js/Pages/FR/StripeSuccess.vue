<template>
    <div class="bg-base-200 px-2 lg:px-0 py-8">
        <div class="bg-white shadow-lg md:w-2/3 lg:w-1/3 mx-auto p-4 rounded-xl">
            <ion-icon class="mx-auto block py-2 text-6xl text-gray-700"
                      :name="data.status === 'succeeded' || data.status === 'processing' ? 'checkmark-circle-outline' : 'close-circle-outline'"></ion-icon>
            <h1 class="text-center pt-3 font-bold text-xl text-gray-700 mb-2">Payment Effectué!</h1>
            <p class="text-center" v-if="data.status === 'succeeded'">Success! Payment received.</p>
            <p class="text-center" v-else-if="data.status === 'processing'">Payment processing. We'll update you when
                the payment is
                received.</p>
            <p class="text-center" v-else-if="data.status === 'requires_payment_method'">Payment failed. Please try
                another payment
                method.</p>
